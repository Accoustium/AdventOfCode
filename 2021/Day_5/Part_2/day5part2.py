...
class VENTLINE(namedtuple("VENTLINE", "x1 y1 x2 y2")):
    ...
    def change_in_x(self) -> Union[bool, int]:
        try:
            change = abs((self.x2 - self.x1) / (self.y2 - self.y1))
            return change if (self.x2 - self.x1) >= 0 else change * -1.0
        except ZeroDivisionError:
            return 1.0 if self.x1 < self.x2 else -1.0

    @property
    def change_in_y(self) -> Union[bool, int]:
        try:
            change = abs((self.y2 - self.y1) / (self.x2 - self.x1))
            return change if (self.y2 - self.y1) >= 0 else change * -1.0
        except ZeroDivisionError:
            return 1.0 if self.y1 < self.y2 else -1.0
...
class HydrothermalVents:
    ...
    def update_vent(self, vent: VENTLINE):
        ...
        while start != (vent.x2 - 1.0, vent.y2 - 1.0):
...
def read_data() -> List[VENTLINE]:
    data = []
    with open("day5part2.txt", 'r') as file:
        for line in file:
            data.append(
                VENTLINE(
